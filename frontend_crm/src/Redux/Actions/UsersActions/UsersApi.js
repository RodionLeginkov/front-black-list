...
async function getData(url, filter,sort, token) {
  const response = await axios.get(url, { params: {filter:filter,sort:sort},headers: { authorization: token } });
  return response;
}
...
export const loadAllUsers = (filter,sort, token) => getData(`${process.env.REACT_APP_BASE_API}users/`, filter,sort, token);
